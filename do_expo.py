...
def expo_apply(sft_dir, dpo_dir, alpha, out_dir):
    ...
    if len(sft_files) != len(dpo_files):
        raise ValueError("SFT and DPO must have same number of shard files")

    os.makedirs(out_dir, exist_ok=True)
    total = len(dpo_files)

    for i, (sft_file, dpo_file) in enumerate(zip(sft_files, dpo_files), 1):
        print(f"Processing shard {i}/{total}: {os.path.basename(dpo_file)}")
        shard_sd = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", default="expo_model")  # 注意：现在是目录！
